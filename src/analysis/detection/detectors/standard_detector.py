...
class StandardDetector(Detector):
    ...
    def get_recording_events(self, predictions, recording_id, options=None):
        ...
        end_threshold = options.get(
            "end_threshold", self.DEFAULT_END_THRESHOLD)
        min_duration = options.get(
        ...
        for pred_time, activity in predictions.itertuples(index=False):
            ...
            if activity > min_activity:
                ...
                if (end - start) > min_duration:
                    events.append({"event_index": event_index, "recording_id": recording_id,
                                   "start": start, "end": end})
    ...
    def get_matches(self, events, tags):
        ...
        unmatched = tags.loc[~tags.tag_id.isin(
            matched.tag_id.unique())]
    ...
    def match_predictions(self, predictions, events, tags):
        for _, x in tags.iterrows():
            predictions.loc[predictions.time.between(x["tag_start"], x["tag_end"]), [
                "tag", "tag_index"]] = [1, x["tag_index"]]

        for _, x in events.iterrows():
            predictions.loc[predictions.time.between(
    ...
    def get_stats(self, events, matches):
        ...
        true_pos_events = len(
            matches[matches.event_id != -1].event_id.unique())
        true_pos_tags = len(
            matches[matches.event_id != -1].tag_id.unique())
        ...
        n_tags_matched = len(
            matches.loc[matches.event_id != -1].tag_id.unique())
        ...
        return {"n_events": events.shape[0],
    ...
    def evaluate(self, predictions, tags, options):
        ...
        # events = self.associate_recordings(events, recordings)
