...
from gui.widgets.options.ui.export_table_options_ui import Ui_ExportTableOptions
...
class ExportTableOptions(QWidget, Ui_ExportTableOptions):
    ...
    def link_events(self):
        ...
        # Table columns
        self.list_columns.selection_changed.connect(self.update_columns)
        ...
        # Expand columns
        self.radiogroup_expand.buttonToggled.connect(self.show_expand_widget)
        self.combo_expand_table.currentIndexChanged.connect(
            self.init_expand_columns_list
        )
        self.list_expand_columns.selection_changed.connect(self.update_expand_columns)

        self.btn_expand_select_all.clicked.connect(self.list_expand_columns.select_all)
        self.btn_expand_deselect_all.clicked.connect(
            self.list_expand_columns.deselect_all
        )
        self.btn_expand_select_required.clicked.connect(self.select_expand_required)
    ...
    def init_ui(self):
        ...
        file_chooser_options = {
            "file_type": "save_file",
            "default": str(
                pathlib.Path.home() / (self.table_name + qApp.dbmanager.FILE_EXTENSION)
            ),
        }
        ...
        if not self.table_model.REFERS_TO:
            self.expand_frame.setHidden(True)
        else:
            if self.radio_expand.isChecked():
                self.init_expand_widget()

    def show_expand_widget(self):
        self.expand_widget.setHidden(self.radio_backup.isChecked())
    ...
    def update_expand_label(self):
        ...
        lbl_text = lbl_text.replace("{1}", ",".join(self.table_model.REFERS_TO.keys()))
    ...
    def update_expand_columns(self):
        self.expand_columns[
            self.combo_expand_table.currentText()
    ...
    def get_options(self):
        opts = {
            ...
            "expand": self.radio_expand.isChecked(),
