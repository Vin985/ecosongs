...
# inc_start = barw_nest.incubation_date_corrected.min().dayofyear
# inc_end = barw_nest.incubation_date_corrected.max().dayofyear
# inc_lbl_pos = inc_start + (inc_end - inc_start)/2
# hatch_start = barw_nest.est_hatch_date.min().dayofyear
# hatch_end = barw_nest.est_hatch_date.max().dayofyear
# hatch_lbl_pos = hatch_start + (hatch_end - hatch_start)/2
...
(ggplot(data=res2, mapping=aes(x='julian', y='ACI_mean', colour='site'))
    ...
    + annotate("rect", xmin=[inc_start,hatch_start], xmax=[inc_end,hatch_end],
        ymin=-math.inf, ymax=math.inf, alpha = 0.2, fill=["red","blue"])
    ...
    + geom_errorbar(aes(ymin="ACI_mean - ACI_std", ymax="ACI_mean + ACI_std"))
    + geom_smooth(method="mavg", se=False, method_args={"window": 4, "center": True, "min_periods": 1})
    + scale_x_continuous(labels=label_x)) #.save("figs/ACI_BARW_inc_hatch.png", height=8, width=8, dpi=150)
