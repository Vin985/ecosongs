...
def compute_spectrogram(file,
    ...
    if square:
        spectro = [
            abs(np.fft.rfft(frame, windowLength))[0:windowLength / 2]**2
            for frame in frames
        ]
    else:
        spectro = [
            abs(np.fft.rfft(frame, windowLength))[0:windowLength / 2]
            for frame in frames
        ]
...
def compute_BI(spectro, frequencies, min_freq=2000, max_freq=8000):
    ...
    area = np.sum(
        spectre_BI_mean_segment_normalized / (frequencies[1] - frequencies[0])
...
def compute_spectral_centroid(spectro, frequencies):
    ...
    centroid = [
        np.sum(magnitudes * frequencies) / np.sum(magnitudes)
        for magnitudes in spectro.T
    ]
